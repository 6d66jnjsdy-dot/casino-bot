...
function parseBet(user, raw) {
  ...
  if (!Number.isFinite(bet) || bet < MIN_BET) {
    return { error: `❌ Minimum bet is **${money(MIN_BET)}**${db.currency}. You can use an exact amount, \`half\`, or \`all\`.` };
  }
  bet = Math.floor(bet);
  ...
  try {
    ...
    if (["lb","leaderboard","top"].includes(command)) {
      ...
      const list = Object.entries(db.users)
        .sort((a, b) => (cashOnly ? b[1].cash - a[1].cash : (b[1].cash + b[1].bank) - (a[1].cash + a[1].bank)))
        .slice(0, 10);
...
function handValue(hand) {
  let t = hand.reduce((s, c) => s + c.number, 0);
  let a = hand.filter(c => c.value === "A").length;
  while (t > 21 && a-- > 0) t -= 10;
  return t;
}
...
function dealPlayerHand() {
  if (Math.random() < 0.234) {
    ...
    return shuffle([makeCard("A", 11), makeCard(tc[0], tc[1])]);
  }
...
async function blackjack(message, args, user) {
  ...
  async function finish(result, payout, color) {
    ...
    saveData();
    await gm.edit({
      embeds: [embed(`**Your Hand**\n${handText(player)} — **${handValue(player)}**\n\n**Dealer**\n${handText(dealer)} — **${handValue(dealer)}**\n\n${result}${payout ? `\nPayout: **${money(payout)}**${db.currency}` : ""}`, color, "🃏 Blackjack 🃏")],
  ...
  collector.on("collect", async i => {
    ...
    if (a === "double") {
      if (user.cash < bet) return i.reply({ content: "❌ Not enough cash.", ephemeral: true });
      user.cash -= bet;
      totalBet += bet;
      player.push(drawStandardCard());
      await i.deferUpdate();
      if (handValue(player) > 21) return finish("💥 Bust!", 0, COLOR_LOSE);
      while (handValue(dealer) < 17) dealer.push(drawStandardCard());
    ...
    if (a === "hit") {
      ...
      return i.update({ embeds: [gameEmbed()], components: [row] });
    }
...
async function coinflip(message, args, user) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`ht:h:${message.author.id}`).setLabel("Heads").setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId(`ht:t:${message.author.id}`).setLabel("Tails").setStyle(ButtonStyle.Success)
  );
  ...
  c.on("collect", async i => {
    ...
    saveData();
    await i.update({
      embeds: [embed(`${result === "h" ? "🪙 Heads" : "🪙 Tails"}\n\n${win ? `🎉 Won **${money(bet * 2)}**${db.currency}!` : `❌ Lost **${money(bet)}**${db.currency}.`}`, win ? COLOR_WIN : COLOR_LOSE, "🍀 CoinFlip 🍀")],
      components: [disabledRow(row)]
    });
  });
...
function hlMultipliers(current) {
  const higher = 100 - current, lower = current - 1;
  return {
...
async function higherLower(message, args, user) {
  ...
  const msg = await message.reply({
    embeds: [embed(`🎲 Higher or Lower 🎲\n\nBetting Amount: ${money(bet)}\n1 : ${current}\n2 : ❓\n\nHigher: **${mult.higher}x**\nSame: **${mult.same}x**\nLower: **${mult.lower}x**`, COLOR_NEUTRAL)],
    components: [row]
  });
  ...
  c.on("collect", async i => {
    ...
    const choice = i.customId.split(":")[1];
    const win = (choice === "hi" && next > current) || (choice === "lo" && next < current) || (choice === "eq" && next === current);
    const m = choice === "hi" ? mult.higher : choice === "lo" ? mult.lower : mult.same;
    const payout = win ? Math.floor(bet * m) : 0;
    if (payout) user.cash += payout;
    saveData();
    await i.update({
      embeds: [embed(`1 : ${current}\n2 : ${next}\n\n${win ? `🎉 Won **${money(payout)}**${db.currency}!` : `❌ Lost **${money(bet)}**${db.currency}.`}`, win ? COLOR_WIN : COLOR_LOSE, "🎲 Higher or Lower 🎲")],
      components: [disabledRow(row)]
    });
  });
...
async function mines(message, args, user) {
  ...
  const bomb = random(0, 15);
  const revealed = new Set();
  let finished = false;

  function mult() {
    return MINES_MULTIPLIERS[Math.min(revealed.size - 1, MINES_MULTIPLIERS.length - 1)] || 1.1;
  }
...
function buildGoldmineBoard() {
  const ids = shuffle([...Array(24).keys()]), b = new Array(24);
  let c = 0;
...
async function roulette(message, args, user) {
  ...
  if ((!isNum && !['red', 'black', 'green'].includes(choice)) || (isNum && (Number(choice) < 0 || Number(choice) > 36))) {
    return message.reply({ embeds: [embed("❌ Usage: `$roulette <amount|half|all> <red/black/green/0-36>`", COLOR_LOSE)] });
  }

  user.cash -= bet;
  saveData();
  const result = random(0, 36), color = rouletteColor(result);
  const win = isNum ? Number(choice) === result : choice === color;
  const mult = isNum ? 30 : (color === "green" ? 14 : 2);
...
async function wheel(message, args, user) {
  ...
  if (bet === null) return;
  user.cash -= bet;
  const r = weightedPick(WHEEL_SEGMENTS), p = Math.floor(bet * r.mult);
  if (p) user.cash += p;
  saveData();
  return message.reply({ embeds: [embed(`🎡 The wheel lands on **${r.label}**\n\n${p ? `🎉 Won **${money(p)}**${db.currency}!` : `❌ Lost **${money(bet)}**${db.currency}.`}`, p ? COLOR_WIN : COLOR_LOSE, "🎡 Wheel of Fortune 🎡")] });
}

/* ============================== CRASH ========================= */
function rollCrashPoint() {
  const r = Math.random(), point = 0.98 / (1 - r * 0.94);
...
async function crash(message, args, user) {
  ...
  const interval = setInterval(async () => {
    if (finished) return;
    mult = Math.round(mult * 1.15 * 100) / 100;
    if (mult >= crashPoint) {
      finished = true;
      clearInterval(interval);
      c.stop();
      saveData();
      await msg.edit({ embeds: [embed(`💥 Crashed at **${crashPoint.toFixed(2)}x**!\n\nLost **${money(bet)}** ${db.currency}.`, COLOR_LOSE, "🚀 Crash 🚀")], components: [disabledRow(row)] }).catch(() => {});
      return;
    }
  ...
  c.on("end", async () => {
    ...
    user.cash += bet;
    saveData();
    await msg.edit({ embeds: [embed(`⏰ Timed out. Returned **${money(bet)}** ${db.currency}.`, COLOR_NEUTRAL, "🚀 Crash 🚀")], components: [disabledRow(row)] }).catch(() => {});
  });
}
...
async function summer(message, user) {
  ...
  if (left > 0) return message.reply({ embeds: [embed(`⏳ Your Summer wheel is ready again in **${formatDuration(left)}**.`, COLOR_LOSE, "☀️ Summer ☀️")] });
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`summer:spin:${message.author.id}`).setLabel("☀️ SPIN").setStyle(ButtonStyle.Primary)
  );

  const msg = await message.reply({
    embeds: [embed(`☀️ **SUMMER DAILY WHEEL** ☀️\n\n🎁 1,750,000 — 45%\n🎁 25,000,000 — 30%\n🎁 65,000,000 — 15%\n🏆 100,000,000 JACKPOT — 5%\n\nPress **SPIN**. You get one spin every 24 hours.`, COLOR_PURPLE, "☀️ Summer ☀️")],
    components: [row]
  });
  ...
  c.on("collect", async i => {
    ...
    for (let n = 0; n < 8; n++) {
      ...
      await msg.edit({
        embeds: [embed(`☀️ **SUMMER WHEEL**\n\n🔄 ${["1,750,000", "25,000,000", "65,000,000", "100,000,000 JACKPOT"][n % 4]}\n\n🎡 Spinning...`, COLOR_PURPLE, "☀️ Summer ☀️")],
        components: []
      }).catch(() => {});
    }
    user.cash += result.amount;
    saveData();
    await logEvent(message.guild, `☀️ Summer result for <@${message.author.id}>: **${result.label}** ${db.currency}.`, COLOR_WIN);
    await msg.edit({
      embeds: [embed(`🎉 **THE WHEEL STOPPED!**\n\n🏆 Prize: **${result.label}** ${db.currency}\n\nYour new cash: **${money(user.cash)}** ${db.currency}.`, COLOR_WIN, "☀️ Summer ☀️")],
      components: []
    }).catch(() => {});
  });
}
